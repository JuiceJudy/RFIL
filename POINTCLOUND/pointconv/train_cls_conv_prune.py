...
from utils.utils import test, save_checkpoint, save_pruned_checkpoint
...
def parse_args():
    ...
    parser.add_argument('--pruned', type=str, default=None,help='pruned model')
...
def main(args):
    ...
    '''CREATE DIR'''
    experiment_dir = Path('./experiment/pruned_retrain/')
    ...
    '''MODEL LOADING'''
    num_class = 40
    logger.info('Load pruned model')
    checkpoint = torch.load(args.pruned)
    pruning_rate = checkpoint['pruning_rate']
    classifier = PointConvClsSsgPrune(num_class,pruning_rate=pruning_rate).cuda()
