...
def sample_and_group(npoint, nsample, xyz, points, density_scale = None):
    ...
    if points is not None:
        ...
        # print('new_points',grouped_xyz_norm.shape,new_points.shape)
    else:
        new_points = grouped_xyz_norm

    # print('++++',xyz.shape,new_points.shape,grouped_xyz.shape,idx.shape,grouped_points.shape)
...
class PointConvDensitySetAbstraction(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        # tmp = torch.matmul(input=new_points.permute(0, 3, 1, 2), other = weights.permute(0, 3, 2, 1))

        # print('----->',torch.matmul(input=new_points.permute(0, 3, 1, 2), other = weights.permute(0, 3, 2, 1)).shape)
